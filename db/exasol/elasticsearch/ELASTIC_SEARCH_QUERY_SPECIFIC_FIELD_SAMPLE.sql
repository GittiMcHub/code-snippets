CREATE OR REPLACE JAVA SCALAR SCRIPT ELASTIC_SEARCH_QUERY_SPECIFIC_FIELD_SAMPLE ("searchField" VARCHAR(50) UTF8, "searchParam" VARCHAR(255) UTF8) RETURNS VARCHAR(100) UTF8 AS
/*
	Creates and ElasticSearch Post Query 
	queries the field of the parameter
...
class ELASTIC_SEARCH_QUERY_SPECIFIC_FIELD_SAMPLE {
	...
	static String run(final ExaMetadata exa, final ExaIterator ctx) throws Exception {
		...
		String elasticDocSearchField = ctx.getString("searchField");

		if(ctx.getString("searchField") == null || ctx.getString("searchParam") == null || ctx.getString("searchField") .trim().length() == 0 || ctx.getString("searchParam") .trim().length() == 0){
			/* Invalid Input */
			return "-2"; 
		}
